...
def load_data(size: int, latency_s: float = slowness) -> np.ndarray:
    time.sleep(latency_s * 5)  # pretend we're fetching from remote storage
...
# 3) Pass the future + extra arg into a function that splits work via worker_client
